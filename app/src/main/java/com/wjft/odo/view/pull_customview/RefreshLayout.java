...
import com.wjft.odo.R;
   ...
   /*
 1.设置刷新头（有默认的还可以自己进来），在onMeasure里面进行测量，在onLayout里面进行布局。
 设置刷新头需要对其layoutparams进行设置，即宽高
 onMeasure里面设置该view的第一个子view（target）的测量和对header的测量，测量完成之后，就能获取到header的高度，（也就是滑动多少距离才能够进行刷新）
 onLayout设置header摆放在target上面，保证target默认在最顶上

 2.事件分发：在dispatchTouchEvent里面设置处理下拉事件时，return true;    交给子view处理时   super.dispatchTouchEvent(ev);
 需要处理的地方：2.1：target到顶部的时候进行往下滑    2.2已经出现了header的时候往上滑
 判断： ViewCompat.canScrollVertically(target, -1);
 通知子view cancel：
 ...
 3.自动回滚：可以采用属性动画，scroller。
 此处采用的是一个内部类，封装该功能，

 4.添加刷新状态：
 设置4中状态：reset，初始化时    pull 下拉   loading   刷新中    complete  完成
 写一个改变状态的方法，同时也可以供自定义header使用
...
public class RefreshLayout extends ViewGroup {
    ...
    // scroller duration
    private static final int SCROLL_TO_TOP_DURATION = 800;//300-0
    private static final int SCROLL_TO_REFRESH_DURATION = 250;//0-300    超出300回到300
